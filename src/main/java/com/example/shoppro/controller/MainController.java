...
public class MainController {
    ...
    //page현재페이지도 받고 //size도 받고 // Pageable 도 페이징처리를 위해서 받고
    @GetMapping(value = "/")
    public String main(PageRequestDTO pageRequestDTO, Model model, Principal principal) {    // , Optional<Integer> page

//        Pageable pageable = PageRequest.of(page.isPresent() ? page.get() : 0, 10);
//        Pageable pageable = PageRequest.of(pageRequestDTO.getPage(), 10);
        // 기존에 만들었던 list는 pageable과 email을 받음으로
        // 자신이 만든 item들의 list만 받아왔다.
        // 그러면 email이 null 그러니까 로그인을 안한사람은 못 본다
        // 그래서 list를 추가로 만들던가 아니면 null일 경우와 null이 아닐경우에 따라서 동적인 쿼리를 만들면 된다.
